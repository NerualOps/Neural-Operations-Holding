...
class EpsilonLanguageEngine {
  ...
  async generate(options = {}) {
    ...
    // Just pass the user message - the inference service handles formatting
    const formattedPrompt = userMessage;

    // Build persona hint for temperature adjustment (model has learned patterns)
    const personaHint = options.persona || this.buildPersonaHint(userMessage, []);
    ...
    try {
      ...
      if (result && result.text) {
        ...
        // Validate response quality - be lenient for base model before fine-tuning
        // Base models can generate short but valid conversational responses
          const genericPatterns = [
          /^(what|how|why|when|where|who)\s+(is|are|do|does|can|will)\s+(this|that|it)\s*$/i,
          /^(i|i'm|i am)\s+(not sure|unsure|not certain)\s*$/i,
          ];
          
          const isGeneric = genericPatterns.some(pattern => pattern.test(responseText.trim()));
        const isTooShort = responseText.length < 10; // Lowered from 50 - allow short responses
        const hasNoSubstance = responseText.trim().length < 5; // Only reject if almost empty
        
        // Only reject if it's clearly broken (almost empty) or a very specific generic pattern
        // Allow short conversational responses like "Hi!" or "Hello! How can I help?"
        if (isTooShort || hasNoSubstance || (isGeneric && responseText.length < 20)) {
          console.warn('[EPSILON AI LANGUAGE ENGINE] Generated response rejected:', responseText.substring(0, 50));
          return null;
          }
